...
impl RevisionMatch {
    pub fn modify_headers(&self, headers: &mut HeaderMap) {
        if let Some(date) = self.modified_since_date {
            let hv = format!("{}", date.format("%c"));
            let hv = HeaderValue::from_str(&hv).unwrap();
    ...
    pub fn modified_since_date(&self) -> Option<NaiveDateTime> {
        self.modified_since_date
    }
    
    pub fn set_modified_since_date(&mut self, modified_since_date: Option<NaiveDateTime>) {
    ...
    pub fn unmodified_since_date(&self) -> Option<NaiveDateTime> {
        self.unmodified_since_date
    }
    
    pub fn set_unmodified_since_date(&mut self, unmodified_since_date: Option<NaiveDateTime>) {
...
mod tests {
    ...
    fn test_revision_match() {
        ...
        revision_match.set_modified_since_date(Some(NaiveDate::from_ymd_opt(2021, 1, 1).unwrap().and_hms_opt(0, 0, 0).unwrap()));
        assert_eq!(revision_match.modified_since_date(), Some(NaiveDate::from_ymd_opt(2021, 1, 1).unwrap().and_hms_opt(0, 0, 0).unwrap()));
        
        revision_match.set_unmodified_since_revisions(vec![4, 5, 6]);
        assert_eq!(revision_match.unmodified_since_revisions(), &[4, 5, 6]);
        
        revision_match.set_unmodified_since_date(Some(NaiveDate::from_ymd_opt(2021, 1, 2).unwrap().and_hms_opt(0, 0, 0).unwrap()));
        assert_eq!(revision_match.unmodified_since_date(), Some(NaiveDate::from_ymd_opt(2021, 1, 2).unwrap().and_hms_opt(0, 0, 0).unwrap()));
        
        revision_match.set_if_match(vec!["1".to_string(), "2".to_string()]);
        assert_eq!(revision_match.if_match(), &["1".to_string(), "2".to_string()]);
        
        revision_match.set_if_none_match(vec!["3".to_string(), "4".to_string()]);
        assert_eq!(revision_match.if_none_match(), &["3".to_string(), "4".to_string()]);
    }
}