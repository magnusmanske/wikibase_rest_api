...
pub enum StatementValueContent {
    String(String),
    Time{time: String, precision: u8, calendarmodel: String},
    Location{latitude: f64, longitude: f64, precision: f64, globe: String},
...
impl StatementValueContent {
    ...
    pub fn from_json(j: &Value) -> Result<Self, RestApiError> {
        ...
        if let (Some(time),Some(precision), Some(calendarmodel)) = (j["time"].as_str(), j["precision"].as_u64(), j["calendarmodel"].as_str()) {
            return Ok(StatementValueContent::Time{time: time.to_string(), precision: precision as u8, calendarmodel: calendarmodel.to_string()});
        }
        if let (Some(latitude), Some(longitude), Some(precision), Some(globe)) = (j["latitude"].as_f64(), j["longitude"].as_f64(), j["precision"].as_f64(), j["globe"].as_str()) {
            return Ok(StatementValueContent::Location{latitude, longitude, precision, globe: globe.to_string()});
        }
        if let (Some(amount), Some(unit)) = (j["amount"].as_str(), j["unit"].as_str()) {
            return Ok(StatementValueContent::Quantity{amount: amount.to_string(), unit: unit.to_string()});
        }
        if let (Some(language), Some(text)) = (j["language"].as_str(), j["text"].as_str()) {
            return Ok(StatementValueContent::MonolingualText{language: language.to_string(), text: text.to_string()});
        }
...
impl Serialize for StatementValueContent {
    ...
    {
        match &self {
            StatementValueContent::String(text) => {
                json!(text).serialize(serializer)
            }
            StatementValueContent::Time { time, precision, calendarmodel } => {
            ...
            StatementValueContent::Location { latitude, longitude, precision, globe } => {
...
impl StatementValue {
    ...
    pub fn from_json(j: &Value) -> Result<Self, RestApiError> {
        ...
        .as_str()
            .ok_or_else(|| RestApiError::MissingOrInvalidField { field: "type".into(), j: j.to_owned() })?;
        match value_type {
            "value" => Ok(Self::Value(StatementValueContent::from_json(&j["content"])?)),
...
mod tests {
    ...
    fn test_serialize_time() {
        let s = StatementValue::Value(StatementValueContent::Time{time: "+2021-01-01T00:00:00Z".to_string(), precision: 11, calendarmodel: "http://www.wikidata.org/entity/Q1985727".to_string()});
        let j: Value = json!(s);
        assert_eq!(j, json!({"type": "value", "content": {"time": "+2021-01-01T00:00:00Z", "precision": 11, "calendarmodel": "http://www.wikidata.org/entity/Q1985727"}}));
    }

    #[test]
    fn test_serialize_location() {
        let s = StatementValue::Value(StatementValueContent::Location{latitude: 37.786971, longitude: -122.399677, precision: 0.0001, globe: "http://www.wikidata.org/entity/Q2".to_string()});
        let j: Value = json!(s);
        assert_eq!(j, json!({"type": "value", "content": {"latitude": 37.786971, "longitude": -122.399677, "precision": 0.0001, "globe": "http://www.wikidata.org/entity/Q2"}}));
    }

    #[test]
    fn test_serialize_quantity() {
        let s = StatementValue::Value(StatementValueContent::Quantity{amount: "42".to_string(), unit: "http://www.wikidata.org/entity/Q11573".to_string()});
        let j: Value = json!(s);
        assert_eq!(j, json!({"type": "value", "content": {"amount": "42", "unit": "http://www.wikidata.org/entity/Q11573"}}));
    }

    #[test]
    fn test_serialize_monolingual_text() {
        let s = StatementValue::Value(StatementValueContent::MonolingualText{language: "en".to_string(), text: "foo".to_string()});
        let j: Value = json!(s);
        assert_eq!(j, json!({"type": "value", "content": {"language": "en", "text": "foo"}}));
    }
    ...
    fn test_from_string() {
        ...
        assert_eq!(s, StatementValue::Value(StatementValueContent::String("foo".to_string())));
    }

    #[test]
    fn test_from_time() {
        let s = StatementValue::Value(StatementValueContent::Time{time: "+2021-01-01T00:00:00Z".to_string(), precision: 11, calendarmodel: "http://www.wikidata.org/entity/Q1985727".to_string()});
        assert_eq!(s, StatementValue::Value(StatementValueContent::Time{time: "+2021-01-01T00:00:00Z".to_string(), precision: 11, calendarmodel: "http://www.wikidata.org/entity/Q1985727".to_string()}));
    }

    #[test]
    fn test_from_location() {
        let s = StatementValue::Value(StatementValueContent::Location{latitude: 37.786971, longitude: -122.399677, precision: 0.0001, globe: "http://www.wikidata.org/entity/Q2".to_string()});
        assert_eq!(s, StatementValue::Value(StatementValueContent::Location{latitude: 37.786971, longitude: -122.399677, precision: 0.0001, globe: "http://www.wikidata.org/entity/Q2".to_string()}));
    }

    #[test]
    fn test_from_quantity() {
        let s = StatementValue::Value(StatementValueContent::Quantity{amount: "42".to_string(), unit: "http://www.wikidata.org/entity/Q11573".to_string()});
        assert_eq!(s, StatementValue::Value(StatementValueContent::Quantity{amount: "42".to_string(), unit: "http://www.wikidata.org/entity/Q11573".to_string()}));
    }

    #[test]
    fn test_from_monolingual_text() {
        let s = StatementValue::Value(StatementValueContent::MonolingualText{language: "en".to_string(), text: "foo".to_string()});
        assert_eq!(s, StatementValue::Value(StatementValueContent::MonolingualText{language: "en".to_string(), text: "foo".to_string()}));
    }
    ...
    fn test_from_json_time() {
        ...
        let s = StatementValueContent::from_json(&j).unwrap();
        assert_eq!(s, StatementValueContent::Time{time: "+2021-01-01T00:00:00Z".to_string(), precision: 11, calendarmodel: "http://www.wikidata.org/entity/Q1985727".to_string()});
    }
    ...
    fn test_from_json_location() {
        ...
        let s = StatementValueContent::from_json(&j).unwrap();
        assert_eq!(s, StatementValueContent::Location{latitude: 37.786971, longitude: -122.399677, precision: 0.0001, globe: "http://www.wikidata.org/entity/Q2".to_string()});
    }
    ...
    fn test_from_json_quantity() {
        ...
        let s = StatementValueContent::from_json(&j).unwrap();
        assert_eq!(s, StatementValueContent::Quantity{amount: "42".to_string(), unit: "http://www.wikidata.org/entity/Q11573".to_string()});
    }
    ...
    fn test_from_json_monolingual_text() {
        ...
        let s = StatementValueContent::from_json(&j).unwrap();
        assert_eq!(s, StatementValueContent::MonolingualText{language: "en".to_string(), text: "foo".to_string()});
    }
    ...
    fn test_statement_value_contents_serialize() {
        ...
        let svc = StatementValueContent::Time{time: "+2021-01-01T00:00:00Z".to_string(), precision: 11, calendarmodel: "http://www.wikidata.org/entity/Q1985727".to_string()};
        let j: Value = serde_json::to_value(&svc).unwrap();
        assert_eq!(j, json!({"time": "+2021-01-01T00:00:00Z", "precision": 11, "calendarmodel": "http://www.wikidata.org/entity/Q1985727"}));

        let svc = StatementValueContent::Location{latitude: 37.786971, longitude: -122.399677, precision: 0.0001, globe: "http://www.wikidata.org/entity/Q2".to_string()};
        let j: Value = serde_json::to_value(&svc).unwrap();
        assert_eq!(j, json!({"latitude": 37.786971, "longitude": -122.399677, "precision": 0.0001, "globe": "http://www.wikidata.org/entity/Q2"}));

        let svc = StatementValueContent::Quantity{amount: "42".to_string(), unit: "http://www.wikidata.org/entity/Q11573".to_string()};
        let j: Value = serde_json::to_value(&svc).unwrap();
        assert_eq!(j, json!({"amount": "42", "unit": "http://www.wikidata.org/entity/Q11573"}));

        let svc = StatementValueContent::MonolingualText{language: "en".to_string(), text: "foo".to_string()};
        let j: Value = serde_json::to_value(&svc).unwrap();
