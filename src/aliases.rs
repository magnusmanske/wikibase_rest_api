...
use crate::{EditMetadata, EntityId, HeaderInfo, HttpGet, HttpMisc, RestApi, RestApiError, RevisionMatch};
...
impl Aliases {
    ...
    /// Constructs a new `Aliases` object from a language code and a JSON array of (string) aliases.
    pub fn from_json_header_info<S: Into<String>>(language: S, j: &Value, header_info: HeaderInfo) -> Result<Self, RestApiError> {
        let language = language.into();
        if language.trim().is_empty() {
            return Err(RestApiError::EmptyValue("Language".into()));
        }
        let aliases = j.as_array()
            .ok_or_else(|| RestApiError::MissingOrInvalidField { field: "Aliases".into(), j: j.to_owned() })?
            .iter()
            .map(|v| Ok(v.as_str()
                .ok_or_else(|| RestApiError::MissingOrInvalidField { field: "Aliases".into(), j: v.to_owned() })?
                .to_string()))
    ...
    pub async fn post_meta(&self, id: &EntityId, api: &mut RestApi, em: EditMetadata) -> Result<Self, RestApiError> {
        let j = json!({"aliases": self.values});
        let (j, header_info) = self.run_json_query(&id, reqwest::Method::POST, j, api, &em).await?;
...
impl HttpMisc for Aliases {
    fn get_rest_api_path(&self, id: &EntityId) -> Result<String, RestApiError> {
        Ok(format!("/entities/{group}/{id}/aliases/{language}", group = id.group()?, language = self.language))
    }
}
...
impl HttpGet for Aliases {
    async fn get_match(id: &EntityId, language: &str, api: &RestApi, rm: RevisionMatch) -> Result<Self, RestApiError> {
        let path = format!("/entities/{group}/{id}/aliases/{language}", group = id.group()?);
...
mod tests {
    ...
    fn test_aliases_values() {
        let aliases = Aliases::new("en", vec!["Foo".to_string(), "Bar".to_string()]);
        assert_eq!(aliases.values(), &vec!["Foo".to_string(), "Bar".to_string()]);
    }
