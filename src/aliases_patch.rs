...
use crate::{language_strings::LanguageStringsMultiple, patch_entry::PatchEntry, EditMetadata, EntityId, FromJson, HttpMisc, Patch, RestApi, RestApiError};
...
impl AliasesPatch {
    ...
    pub fn replace<S1: Into<String>, S2: Into<String>>(&mut self, language: S1, num: usize, value: S2) {
        <Self as Patch<LanguageStringsMultiple>>::replace(self, &format!("/{}/{num}", language.into()), value.into().into())
    }
    ...
    pub fn remove<S: Into<String>>(&mut self, language: S, num: usize) {
        <Self as Patch<LanguageStringsMultiple>>::remove(self, &format!("/{}/{num}", language.into()))
    }

    /// Generates a patch from JSON, presumably from json_patch
    pub fn from_json(j: &Value) -> Result<Self, RestApiError> {
        let pe = j.as_array()
            .ok_or(RestApiError::MissingOrInvalidField{field: "AliasPatch".to_string(), j: j.clone()})?
            .iter()
...
impl Patch<LanguageStringsMultiple> for AliasesPatch {
    ...
    async fn apply_match(&self, id: &EntityId, api: &mut RestApi, em: EditMetadata) -> Result<LanguageStringsMultiple, RestApiError> {
        ...
        LanguageStringsMultiple::from_json_header_info(&j, header_info)
    }
}
...
mod tests {
    ...
    async fn test_aliases_patch() {
        ...
        Mock::given(body_partial_json(json!({"patch":[{"op": "replace","path": "/en/1","value": new_alias}]})))
            .and(method("PATCH"))
            .and(path(&mock_path))
            .and(bearer_token(token))
            .and(header("content-type", "application/json-patch+json"))
            .respond_with(ResponseTemplate::new(200).insert_header("ETag", "12345").set_body_json(&new_aliases))
        ...
        let new_aliases = patch.apply(&id, &mut api).await.unwrap();
