...
pub trait HttpMisc {
    ...
    async fn run_json_query(&self, id: &EntityId, method: reqwest::Method, j: Value, api: &mut RestApi, em: &EditMetadata) -> Result<(Value,HeaderInfo), RestApiError> {
        let request = self.generate_json_request(id, method, j, api, em).await?;
        let response = api.execute(request).await?;
        self.filter_response_error(response).await
    }

    async fn generate_json_request(&self, id: &EntityId, method: reqwest::Method, mut j: Value, api: &mut RestApi, em: &EditMetadata) -> Result<reqwest::Request, RestApiError> {
        ...
        let mut request = api.wikibase_request_builder(&path, HashMap::new(), method).await?.build()?;
        request.headers_mut().insert(reqwest::header::CONTENT_TYPE, content_type);
        em.revision_match().modify_headers(request.headers_mut());
        *request.body_mut() = Some(format!("{j}").into());
        Ok(request)
    }

    async fn filter_response_error(&self, response: reqwest::Response) -> Result<(Value,HeaderInfo), RestApiError> {
...
pub trait HttpGet: Sized+HttpMisc {
    async fn get_match(id: &EntityId, part_id: &str, api: &RestApi, rm: RevisionMatch) -> Result<Self, RestApiError>;
...
pub trait HttpGetEntity: Sized+HttpMisc {
    async fn get_match(id: &EntityId, api: &RestApi, rm: RevisionMatch) -> Result<Self, RestApiError> where Self: Sized;

    async fn get(id: &EntityId, api: &RestApi) -> Result<Self, RestApiError> where Self: Sized {
...
mod tests {
    ...
    async fn test_filter_response_error() {
        ...
        let response = reqwest::Response::from(http::Response::builder().status(400).body(r#"{"code":"foo","message":"bar"}"#).unwrap());
        let result = sl.filter_response_error(response).await;
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().to_string(), "400 Bad Request Bad Request / foo: bar / {}");
    }
}