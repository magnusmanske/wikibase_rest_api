...
use crate::{EditMetadata, EntityId, HeaderInfo, HttpDelete, HttpGet, HttpMisc, HttpPut, LanguageString, RestApi, RestApiError, RevisionMatch};
...
impl HttpMisc for Label {
    fn get_rest_api_path(&self, id: &EntityId) -> Result<String, RestApiError> {
        Ok(format!("/entities/{group}/{id}/labels/{language}", group = id.group()?, language = self.ls.language()))
    }
}
...
impl HttpGet for Label {
    async fn get_match(id: &EntityId, language: &str, api: &RestApi, rm: RevisionMatch) -> Result<Self, RestApiError> {
        let path = format!("/entities/{group}/{id}/labels/{language}", group = id.group()?);
        ...
        let s = j
            .as_str()
            .ok_or_else(|| RestApiError::MissingOrInvalidField { field: "Label".into(), j: j.to_owned() })?;
        Ok( Self {
...
impl HttpDelete for Label {
    async fn delete_meta(&self, id: &EntityId, api: &mut RestApi, em: EditMetadata) -> Result<(),RestApiError> {
        let j = json!({});
        let (j, _header_info) = self.run_json_query(&id, reqwest::Method::DELETE, j, api, &em).await?;
        match j.as_str() {
